...
- A live process owns `data/wiki-store.json.runtime-owner.json` by default. A launcher that starts a Next.js child also creates `data/wiki-store.json.runtime-owner.json.child.json`. These are exclusive runtime ownership artifacts, not application data or backup artifacts.
...
Every supported launcher performs a storage preflight and exclusively creates an immutable owner file beside `WIKI_STORE_FILE_PATH` before Next.js can accept requests. With the default store this is `data/wiki-store.json.runtime-owner.json`. A launcher never replaces that file during handoff: it exclusively creates `data/wiki-store.json.runtime-owner.json.child.json`, and the spawned Next.js child remains behind a startup gate until both immutable records identify its inherited random lease ID and PID. The assignment is also bound to the exact owner-record contents. This prevents a killed launcher parent from leaving an unrecorded live server and prevents a handoff from overwriting changed or missing ownership. A second process using the same runtime volume exits instead of independently running translation jobs, cache generations, rate and concurrency limits, analytics ingestion, or TLS/ACME coordination. Invoking the Next CLI directly is also rejected.
...
Graceful shutdown removes the child-assignment file, when present, and then the owner file. After any ungraceful stop, startup fails closed for every existing ownership artifact—even a well-formed same-host record whose PID appears dead. PID checks cannot make stale-file replacement atomic, so Vicky never automatically reclaims or deletes these files. First verify that every former Vicky launcher, custom server, and Next.js child is terminated, then remove only the exact owner and child-assignment paths printed by the startup error before restarting. Both files must be absent. Do not restore or copy either file as application data.
